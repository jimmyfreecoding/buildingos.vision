...
为了确保边缘端 AI 在不同业务场景下的鲁棒性与节能平衡，系统实施了以下两层时段控制策略：

### A. 降级保护时段 (定义在 `main.py` 中)
用于在 Gemma 复核重试失败（如模型服务重启中）后，决定最终的“保底”判定结果：

- **上班时段 (09:00 - 19:00)**：倾向于 **有人 (YES)**。如果 Detector 发现了疑似目标，系统宁愿相信有人，防止因为 AI 引擎的小波动导致误关灯，影响办公心流。
- **非上班时段 (19:00 - 次日 09:00)**：倾向于 **无人 (NO)**。为了节能，即便 Detector 发现了低置信度目标，如果 Gemma 无法确认，则强制判定为无人，防止灯光/空调彻夜空转。

### B. 状态机收敛时段 (定义在 `state_machine.py` 中)
用于决定人员感知的“综合判定窗口”时长，实现分时段的消抖策略：

- **上班时段 (WORKTIME) : 09:00 - 19:00**
  - **默认窗口：10 分钟**。在此期间只要有 1 次检测到人，则保持“有人”状态，确保稳定。
- **加班时段 (OVERTIME) : 19:00 - 23:00**
  - **默认窗口：15 分钟**。考虑到加班时段动作幅度通常更小，通过拉长窗口防止误判关灯。
- **深夜时段 (NIGHT) : 23:00 - 次日 09:00**
  - **默认窗口：5 分钟**。深夜主要针对异常闯入，要求响应速度更高，快速判定无人以关灯节能。
